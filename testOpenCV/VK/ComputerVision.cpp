...
int home_width_min = 0;
int away_width_min = 0;
...
int home_width_max = FRAME_HEIGHT/2;
int away_width_max = FRAME_HEIGHT/2;
...
double dist_coeff[5][1] = {  {-0.03647790},
                             {0.0184763},
                             {0.0003989429},
                             {-0.00003392181},
                             {-0.006141144}
...
void calibrateField(VideoCapture capture) {
  ...
  createTrackbar( "Home Goal Width", trackbarWindowName, &home_width_min, home_width_max, on_trackbar );
  createTrackbar( "Away Goal Width", trackbarWindowName, &away_width_min, away_width_max, on_trackbar );
  ...
  setTrackbarPos( "Home Goal Width", trackbarWindowName, home_goal_width);
  setTrackbarPos( "Away Goal Width", trackbarWindowName, away_goal_width);
  ...
  while (1) {
    ...
    home_goal_width = getTrackbarPos( "Home Goal Width", trackbarWindowName);
    away_goal_width = getTrackbarPos( "Away Goal Width", trackbarWindowName);
    ...
    //Goal Home
        Rect homeOutline(0, (field_height/2-home_goal_width/2), 10, home_goal_width);
    rectangle(cameraFeed,homeOutline,Scalar(255,255,255), 1, 8 ,0);
    //Goal Away
        Rect awayOutline(field_width-10, (field_height/2-away_goal_width/2), 10, away_goal_width);
    rectangle(cameraFeed,awayOutline,Scalar(255,255,255), 1, 8 ,0);
...
void * processorThread(void * notUsed){
  ...
  while(1){
    ...
    if (value < MIN_BUFFER_SIZE){
      ...
      frameMat.image = frameRaw.image;//frameMat.image = imdecode(frameRaw.image, CV_LOAD_IMAGE_COLOR);
...
int main(int argc, char* argv[]) {
  ...
  while(1) {
    ...
    //Point left_mid(0, field_height/2);
    //Point right_mid(field_width, field_height/2);
    //C++: void rectangle(cameraFeed, left_mid, Point pt2, const Scalar& color, int thickness=1, int lineType=8, int shift=0)
    //line(cameraFeed,top_mid, bot_mid, Scalar(200,200,200), 1, 8, 0);
    //line(cameraFeed,left_mid, right_mid, Scalar(200,200,200), 1, 8, 0);
